...
export function Player({releaseList}) {
  ...
  const loop = useCallback(() => {
    if (!audioElement.current.paused) { //в условии нельзя установливать isPaused,
      window.requestAnimationFrame(loop);// т.к. его состояние устанавливается после вызова loop
    }
    const array = new Uint8Array(2048);
    analyser.getByteFrequencyData(array);
    // blur.current.style.background = `repeating-radial-gradient(circle, rgb(${array[4]}, ${array[8]}, ${array[16]}) ${array[0]*0.2}%, rgba(${array[32]}, ${array[64]}, ${array[128]}, ${array[24]}) ${array[0]*0.3}%, rgba(${array[16]}, ${array[48]}, ${array[32]}, ${array[32]}) ${array[0]*0.6}%)`//осторожно!!для тех кто хочет сойти с ума)))
    blur.current.style.background = `radial-gradient(at bottom, rgba(${array[4]}, ${array[8]}, 255, 0.7) 10%, rgba(255, ${array[64]}, ${array[128]}, 0.5) 50%, rgba(${array[16]}, ${array[48]}, ${array[32]}, 0.3) 90%), url("../../images/background.jpg")`
  }, [])
