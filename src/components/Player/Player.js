...
export function Player({ releaseList }) {
  ...
  const duration = audioElement.current ? Math.round(audioElement.current.duration) : 0;
  ...
  return(
    <div className={styles.container}>
      <div className={styles.playerContainer}>
        <button onClick={togglePlay} className={styles.playBtn}>{isPaused ? <PlayIcon /> : <PauseIcon />}</button>
        <div className={styles.songContainer}>
          <div className={styles.songContent}>
            <span ref={trackName} className={`${styles.songName} ${isTrackNameOverflow ? styles.songName_overflow : ''}`}>{currentTrack.name}</span>
            <span className={styles.currentTime}>{currentMinutes}:{currentSeconds < 10 ? `0${currentSeconds}` : currentSeconds}</span>
          </div>
          <div className={styles.progressBar}><span className={styles.progress} style={{ width: `${currentTime / duration * 100}%` }}></span></div>
        </div>
        {isOpenPlaylist && <button onClick={toggleLyrics} className={styles.lyricsBtn}>{isOpenLyrics ? 'Релизы' : 'Текст песни'}</button>}
        <button onClick={togglePlaylist} className={styles.togglePlaylist}>{isOpenPlaylist ? <CrossIcon/> : <ArrowIcon/>}</button>
